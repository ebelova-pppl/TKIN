...
23014    continue
23012 continue
      call read_psi_bc(psie,u,zc,rc,rad) 
      u = psie
...
23018    continue
    ...
    1    continue
     ...
     &dhi(iaxis,jaxis) - r0*jphi(iaxis,jaxis) ) )
         ...
         call j_i(sf,jz,jr,jphi,deni,u,hi,rad) 
...
23036 continue
      ...
      close(91)
      if(nz.ne.mynz .or. nr.ne.mynr) then
      ...
      if( myid.eq.master ) print *, 
     &'Reading psi BCs from file=psi_bc.dat'
      if( myid.eq.master ) print *, 'z range=',z_lo,z_hi,'  vs zc=',zc
      if( myid.eq.master ) print *, 'r range=',r_lo,r_hi,'  vs rci,rc=',
     &rci,rc
      open(91,file='psi_bc.dat',status='old')
      read(91,*) mynz,mynr
      read(91,*) z_lo,z_hi,r_lo,r_hi
      ...
      do 23004 i= 2,nz-1
         ...
         do 23006 j= 2,nr-1
            if( u(i,j) .lt. 0. ) then
               ...
               if(j0 .gt. 1) then
                  psie(i,j)= psi_hi(i)*(rad(j)**2 - rad(j0)**2)/(rc**2 -
     & rad(j0)**2)
               else
                  psie(i,j)= psi_lo(i) + (psi_hi(i)-psi_lo(i))*(rad(j)**
     &2-rad(1)**2) /(rc**2 - rad(1)**2)
               endif
            endif
23006    continue
23004 continue
