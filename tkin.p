...
c           v2mo1(i,j,k)= vpert*xx**6*sin(nmode*s(k))
...
c     hi0= bt*0.5*(rad(1)+rad(nr))          # hi value at major radius.
      hi0= bt*21.2                          # TMP for shot 141711_470.
...
# Note: fp1(1)<>0 is assumed.
      ...
      COMMON_FRC
c     parameter(a1=-2.21166,a2=21.1309,a3=-68.6066,a4=102.536)
c     parameter(a5=-72.4683,a6=19.4894)
      real hi(nz,nr), dhi(nz,nr), psi(nz,nr), rad(nr)
      real hhp1(nz,nr), hhp2(nz,nr)
      real sigma, rf1, rf2

c     f1(x)=  a1*x**2 +a2*x**3 +a3*x**4 +a4*x**5 +a5*x**6 +a6*x**7
c     fp1(x)= 2*a1*x +3*a2*x**2 + 4*a3*x**3 +5*a4*x**4 + 6*a5*x**5 +7*a6*x**6
c     f2(x)= x**2*(0.82-x)
c     fp2(x)= 2*x*(0.82-x) - x**2
...
c     MPI_print jb,rb,hb,xim,pminus,pplus
...
c       f2(xa,ya) = exp(-((xa-0.7)/0.3)**2) + 0.25*exp(-(xa/0.3)**2)
...
# 1d integration routine qsimp(NR).
      ...
      do l= 2,lmax
        ...
        if( func < 1.e-7 ) goto 100                 # New 11/15/2019
        dss=ss-os
        os= ss
        ost=st
      enddo l
...
# TMP
c       print *,'=='
c       print *,'sp arrays=',dennsp(1),npsp(1),qnsp(1),tratsp(1),vthplsp(1)
c       print *,wnsp(1),wdqsp(1),wsp(1),qsp(1)
c       print *,'=='
c       print *,'psp(1:5)=',psp(1:5)
c       print *,'############################'
      endif
